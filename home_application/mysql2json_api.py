...
def TableToJson():
    try:
        ...
        cur = conn.cursor()
        # 3.编写sql
        sql = "SELECT a.`business_id` AS bid, a.`business_name` AS bname, GROUP_CONCAT(NAME) FROM applications a GROUP BY a.`business_id` ORDER BY a.`business_name`"
        # 4.执行sql命令
        ...
        for row in data:
            ...
            result['n'] = row[1]
            app_str = str(row[2])
            result['s'] = [{'n': str(x)} for x in app_str.split(',')]
